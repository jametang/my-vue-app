  ...
  <div id="app">
    ...
    <div>
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          <div class="text-center-oblique-large">
            顾客联
          </div>
        </el-col>
      ...
      <el-table :data="tableGoods" style="width: 100%;background-color: blueviolet;"
        :header-row-style="{ height: '50px' }" :row-style="{ height: '50px' }" :cell-style="{ padding: '0px' }">
        <el-table-column prop="Type" label="类型" min-width="8.33%" align="center">
          <template slot-scope="scope">
            <input type="text" v-model="scope.row.Type" v-show="editEnable" />
            <span v-show="!editEnable">{{scope.row.Type}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="Content" label="产品内容" min-width="41.67%" align="center">
          <template slot-scope="scope">
            <input type="text" v-model="scope.row.Content" v-show="editEnable" />
            <span v-show="!editEnable">{{scope.row.Content}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="Unit" label="单位" min-width="8.33%" align="center">
          <template slot-scope="scope">
            <input type="text" v-model="scope.row.Unit" v-show="editEnable" />
            <span v-show="!editEnable">{{scope.row.Unit}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="Num" label="数量" min-width="8.33%" align="center">
          <template slot-scope="scope">
            <input type="text" v-model="scope.row.Num" v-show="editEnable" />
            <span v-show="!editEnable">{{scope.row.Num}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="Prices" label="原价" min-width="8.33%" align="center">
          <template slot-scope="scope">
            <input type="text" v-model="scope.row.Prices" v-show="editEnable" />
            <span v-show="!editEnable">{{scope.row.Prices}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="Sales" label="折扣" min-width="8.33%" align="center">
          <template slot-scope="scope">
            <input type="text" v-model="scope.row.Sales" v-show="editEnable" />
            <span v-show="!editEnable">{{scope.row.Sales}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="AfterSales" label="折扣价" min-width="8.33%" align="center">
          <template slot-scope="scope">
            <input type="text" v-model="scope.row.AfterSales" v-show="editEnable" />
            <span v-show="!editEnable">{{scope.row.AfterSales}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="Sum" label="金额" min-width="8.33%" align="center">
          <template slot-scope="scope">
    ...
    <el-row :gutter="20">
        ...
        <div class="grid-content-200">
          <el-container style="height: 250px; border: 1px solid #eee">
            <el-aside width="25%" style="background-color: blue">
              <el-container style="width: 100%; background-color: cadetblue;">
                ...
                <el-container :style="{ height: '100px' }">
                  <el-aside width="50%" style="border: 1px solid gray">远用</el-aside>
                  <el-main width="50%" style="background-color: white; padding: 0;">
                    <div style="line-height: 50px;text-align: center;border: 1px solid gray">R</div>
                    <div style="line-height: 50px;text-align: center;border: 1px solid gray">L</div>
                  </el-main>
                </el-container>
                <el-container :style="{ height: '100px' }">
                  <el-aside width="50%" style="border: 1px solid gray">近用</el-aside>
                  <el-main width="50%" style="background-color: white; padding: 0;">
                    <div style="line-height: 50px;text-align: center;border: 1px solid gray">R</div>
                    <div style="line-height: 50px;text-align: center;border: 1px solid gray">L</div>
                  </el-main>
            ...
            <el-main width="75%" style="background-color: yellow;padding: 0;">
              <el-table :data="tableData" style="width: 100%;background-color: blueviolet;"
                :header-row-style="{ height: '50px' }" :row-style="{ height: '50px' }" :cell-style="{ padding: '0px' }">
                <el-table-column prop="SphericalMirror" label="球镜" min-width="16.66%" align="center">
                  <template slot-scope="scope">
                    <input type="text" v-model="scope.row.SphericalMirror" v-show="editEnable" />
                    <span v-show="!editEnable">{{scope.row.SphericalMirror}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="Cylinder" label="柱镜" min-width="16.66%" align="center">
                  <template slot-scope="scope">
                    <input type="text" v-model="scope.row.Cylinder" v-show="editEnable" />
                    <span v-show="!editEnable">{{scope.row.Cylinder}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="AxialPos" label="轴位" min-width="16.66%" align="center">
                  <template slot-scope="scope">
                    <input type="text" v-model="scope.row.AxialPos" v-show="editEnable" />
                    <span v-show="!editEnable">{{scope.row.AxialPos}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="CorrectedVision" label="矫正视力" min-width="16.66%" align="center">
                  <template slot-scope="scope">
                    <input type="text" v-model="scope.row.CorrectedVision" v-show="editEnable" />
                    <span v-show="!editEnable">{{scope.row.CorrectedVision}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="InterpupillaryDis" label="瞳距" min-width="16.66%" align="center">
                  <template slot-scope="scope">
                    <input type="text" v-model="scope.row.InterpupillaryDis" v-show="editEnable" />
                    <span v-show="!editEnable">{{scope.row.InterpupillaryDis}}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="DownLight" label="下加光" min-width="16.67%" align="center">
                  <template slot-scope="scope">
                    <input type="text" v-model="scope.row.DownLight" v-show="editEnable" />
      ...
      <el-col :span="6">
        <div class="grid-content-200 padding-48">
          <img src="https://6d65-melo-order-app-env-2dxtm5d9d9602-1251479247.tcb.qcloud.la/phone.png?sign=168483cc4b51556343daee9abe34ff82&t=1661592744" alt="" height="200px" width="200px" />
        </div>
      </el-col>
    </el-row>
    ...
    <div class="grid-content text-left" style="font-size:x-large">地址：XXXXXXXXXXXXXXXXXXXXXXXXXXXXX 电话： XXXXXXXXXXX</div>

    <!-- <div>
      <el-button @click="startHacking">
